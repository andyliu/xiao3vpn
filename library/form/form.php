...
class QForm extends QForm_Group
{
    ...
    function __call($item,$args)
    {
        return $this->render($item,array_shift($args));
    }
    
    function toArray($exclude = '')
    {
        $ret=array();
        foreach($this->_elements as $key=>$val)
        {
            if(!in_array($key,explode(',',$exclude)))
            $ret[$key] = $val->value;
        }
        return $ret;
    }
    ...
    function loadFromConfig(array $config)
    {
        if (isset($config['~form']))
        {
            ...
            {
                $this->_attrs['name'] = $this->_attrs['id'];
            }
    ...
    function start()
    {
        ...
        $str = '<form ';
        foreach ($this->attrs() as $attr => $value): if(!is_array($value)) $str .= "{$attr}=\"{$value}\" ";endforeach;
        $str .='>';
        return $str;     
    }

    function submit($arr = array('value'=>'提交'))
    {
        $attrs ='';
        foreach($arr as $attr => $value) $attrs.= "{$attr}=\"{$value}\" ";
