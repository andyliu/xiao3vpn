...
class Account extends QDB_ActiveRecord_Abstract
{

    function MonthlyTraffic()
    {
        $sql ="SELECT SUM( acctinputoctets + acctoutputoctets ) as total FROM radacct WHERE UserName =  '{$this->username}'";
        $str = QDB::getConn()->getOne($sql);
        return ($str) ? _size($str):'0';
    }

    function lastLogin()
    {
        $sql ="SELECT authdate FROM radpostauth WHERE username =  '{$this->username}' and `reply` = 'Access-Accept' ORDER BY authdate desc";
        $str = QDB::getConn()->getOne($sql);
        return ($str) ? $str:'-';
    }
    ...
    static function __define()
    {
        ...
        (
            ...
            (
                ...
                #'user' => array(QDB::HAS_ONE =>'Account_User','target_key'=>'user_id','source_key'=>'id'),
            ),  
