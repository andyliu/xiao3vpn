...
class Q
{
    ...
    static function cookie($key,$val = NULL)
    {
        ...
        setcookie($key,$val,time()+3864000,'/');
...
function _size($bytes,$precision = 2)
{
...
function __uid()
{
    return substr(md5(microtime()),8,16);
}

function __oid()
{
    list($uec, $sec) = explode(".", microtime(true));
    $sec = str_pad($sec,4,0,STR_PAD_LEFT);
    $str = date('YmdHis') . $sec;
    return $str;
}

function __url($udi = null,$arg = array())
{
    if(is_array($udi)) {$arg = $udi; $udi = null;}
    $context =  QContext::instance();

    $get = $_GET;
    unset($get['module']); unset($get['action']); unset($get['controller']);

    if($udi === false)
        $arg = $arg ? $arg : $get;
    else
        $arg = $arg ? array_merge($get,$arg) : $get;

    $udi = $udi ? $udi : $context->requestUDI(false);

    return url($udi) . '?'.http_build_query($arg, '', '&');
}

function __cut($title,$lenth = 10)
{
...
function _trim($src,$str = ',')
{
    return trim(str_replace($str,'',$src));
}


function ic($str_1,$str_2,$str_3)
{
    if(strpos($str_2,','))
    {
        $str_4 = '';
