...
class Plans extends QDB_ActiveRecord_Abstract
{

    function name($is_admin = false)
    {
        if($is_admin)
        {
            return '<a href="'.url('manage::plans/modify',array('id'=>$this->id())).'">'. $this->name .'</a>';
        }

        return $this->name;
    }

    function getCost($billingcycle = null)
    {
        $sku  = json_decode($this->sku,true);
        
        if(!isset($sku[$billingcycle]))
        {
            return 500;
        }

        $cost = $sku[$billingcycle]['price'];
