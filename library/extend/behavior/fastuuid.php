...
class Model_Behavior_Fastuuid extends QDB_ActiveRecord_Behavior_Abstract
{
    ...
    (
        //! 计算种子数的开始时间
        'being_timestamp' => 1200000000, // 2008-03-27
        //! 计算 ID 时要添加多少位随机数
        'suffix_len' => 1,
        'genorder' => false,
        'gen2char' => false,
    ...
    static function uuid($being_timestamp, $suffix_len)
    {
     ...
     * @return string
     */
    static function genUUID($genorder = false,$being_timestamp, $suffix_len)
    {
        if($genorder)
        return genorder();

        $time = explode(' ', microtime());
        return date('ymd') . sprintf('%07u', substr($time[0], 2, 7));
        $id = ($time[1] - $being_timestamp) . sprintf('%06u', substr($time[0], 2, 6));
        if ($suffix_len > 0)
        {
            $id .= substr(sprintf('%010u', mt_rand()), 0, $suffix_len);
