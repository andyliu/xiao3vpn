...
<table class="datalist" border="0" cellspacing="0" cellpadding="0" width="100%">
    ...
    <tr>
        ...
        <td><input type="hidden" value="<?=$row->id();?>" name="id" /><input type="hidden" value="<?=$type;?>" name="type" /><input type="submit" value="提交" class="btn" /> | <a href="<?=url('manage::radius/modgroup',array('type'=>$type,'deleteid'=>$row->id()));?>">删除</a></td>
    </tr>
    </form>
    <?php endforeach;?>
    <?php else:?>
